...
tags:
  ...
  - git-ls-files-ground-truth
  - readme-command-validator-trap
  - backticked-path-match
  - projectscylla
  - myrmidons
  - hephaestus
---
...
## Sub-Pattern: Docs-Catalog↔Tracked-File-Set Completeness Guard (`git ls-files` ground truth)

> **Warning:** This sub-pattern (added in v1.12.0, from ProjectHephaestus issue #2168 — a scripts-README catalog documenting only a subset of tracked scripts) is **unverified** — it is a PLAN; the guard test was authored but NOT executed, and no CI validated it. The empirical enumeration (`git ls-files` counts, per-file header reads) WAS performed in the planning session. Treat the workflow as a hypothesis until CI confirms.

This is the third doc-drift instance of the membership-guard family, applied to a **CATALOG doc ↔ TRACKED-FILE-SET** invariant: a README section titled "Available Scripts" implicitly claims completeness over `git ls-files scripts/`, but nothing asserted it, so the catalog rotted from complete to 9-of-29 as scripts were added. Unlike the doc-table↔`__all__` case, the source of truth is **git's tracked set**, not a live import surface.

### Proposed Workflow

> **Warning:** This workflow has not been validated end-to-end. Treat as a hypothesis until CI confirms.

1. **Re-derive BOTH numbers in the audit claim empirically.** The issue said "documents only 10 of 31 tracked scripts"; planning-time ground truth was 9 documented of 29 tracked (`git ls-files scripts/ | sort`, minus the README itself). Membership drifts between audit and fix — the drift is itself the defect being guarded. Plan against ground truth and note the discrepancy explicitly (same rule as the `comm -23` mirror case).
2. **Ground truth is `git ls-files <dir>`, NOT a filesystem glob.** A glob false-fails on untracked local scratch files and guards the wrong claim; "tracked" is the claim domain of a repository catalog. Precedent for the technique: an existing guard (`scripts/check_build_dir_untracked.py`) already derives its population from `git ls-files` via subprocess.
3. **Match each entry as a backticked dir-relative path.** Assert `` f"`{relpath}`" in readme_text `` where `relpath` is relative to the cataloged dir (`shell/install.sh`, `slurm/pi_smoke.sbatch`). This makes subdirectory entries exact and the guard implementable as a string check — no markdown parser. The catalog's bullet style (`` **`relpath`** ``) must therefore use the relative path as the display name, not the basename.
4. **Source every one-line description from the file's own header docstring/comment.** Read all M headers; paraphrase, never invent. A catalog whose descriptions are fabricated is worse than an incomplete one — the per-file header is the same "source of truth" discipline as `__all__` for the API-table case.
5. **Place the guard in the sanctioned docs-test dir and make the failure message actionable.** E.g. `tests/unit/docs/test_scripts_readme_catalog.py` (precedent: existing docs-invariant tests like `test_adr_records.py`); the assertion message lists the exact missing relative paths and says what to add. Include a non-empty-population guard (`assert scripts, "git ls-files returned no scripts — guard cannot see its inputs"`) so a broken subprocess can't produce a vacuous pass — the fail-loud-on-empty-input rule from `parser-found-no-rows`.
6. **Never add new fenced command blocks to a README swept by a README-command validator.** The repo's `validate_readme_commands.py` executes commands found in README code blocks; documenting 20 new scripts as runnable snippets would silently enlarge an executable surface with unverified commands. Catalog entries are PROSE BULLETS; the pre-existing verified Usage block stays untouched.
7. **The reverse direction (stale entry for a deleted script) needs a stricter anchor.** The forward guard (`tracked - documented`) fixes the issue's defect. The reverse (`documented - tracked`) cannot scan ALL backticked strings (a README backticks many non-path tokens); it requires extracting only the catalog's `` **`path`** `` bullet anchors and asserting each is tracked. Cheap once the bullet style is fixed by step 3 — plan it as the bidirectional completion, or note its omission explicitly (KISS) so the reviewer sees a decision, not an oversight.
...
| Trusted the audit's "10 of 31" catalog count (ProjectHephaestus #2168, UNVERIFIED plan) | Started planning the catalog fix against the issue's stated counts | Planning-time ground truth was 9 documented of 29 tracked — membership had drifted since the audit, which is exactly the rot being guarded | Re-derive BOTH sides of a "documents N of M" claim from `git ls-files <dir>` at plan time; note the discrepancy explicitly |
| Considered a filesystem glob as the guard's ground truth (ProjectHephaestus #2168, UNVERIFIED plan) | Enumerating the catalog population with `Path.glob` over the scripts dir | Untracked local scratch files would false-fail the guard, and "on disk" is not the claim a repository catalog makes — "tracked" is | Derive the population from `git ls-files <dir>` via subprocess (precedent: `check_build_dir_untracked.py`); the tracked set IS the claim domain |
| Nearly documented new scripts as runnable README snippets (ProjectHephaestus #2168, UNVERIFIED plan) | Adding usage code blocks per newly cataloged script | The repo's `validate_readme_commands.py` EXECUTES commands found in README code blocks — 20 unverified snippets would silently enlarge an executable doc surface and fail CI | Catalog entries are prose bullets with backticked relative paths; leave the pre-existing verified Usage block untouched |
...
**Docs-catalog completeness generalization (v1.12.0, unverified):** A doc section that implicitly claims completeness over a FILE SET ("Available Scripts", a docs index) is a membership invariant whose source of truth is **`git ls-files <dir>`** — the tracked set, never a filesystem glob (untracked scratch false-fails; "tracked" is the claim domain). Guard it with a test in the sanctioned docs-test dir that asserts each tracked file appears as a **backticked dir-relative path** (`` f"`{relpath}`" in readme_text `` — exact for subdirectory entries, no markdown parser needed; the catalog's bullet style must display the relative path, not the basename). Re-derive BOTH numbers of any "documents N of M" audit claim at plan time (membership drifts — the drift IS the defect). Source each one-line description from the file's own header docstring (paraphrase, never invent). Include a fail-loud non-empty-population assert so a broken `git ls-files` subprocess can't vacuously pass. NEVER document scripts via new fenced command blocks in a README swept by a README-command validator (it executes them) — prose bullets only. The reverse direction (stale entry for a deleted file) needs a stricter `` **`path`** `` bullet-anchor parse — plan it as the bidirectional completion or note its omission as an explicit decision. Plan-only (ProjectHephaestus #2168): the enumeration was performed, the guard test was authored but not executed.
...
| ProjectHephaestus | issue #2168 (PLAN — **unverified**) | docs-catalog↔tracked-file-set completeness guard: scripts README claimed "Available Scripts" but documented 9 of 29 tracked files (issue's audit said 10 of 31 — counts drifted, re-derived via `git ls-files scripts/`); plan catalogs all 29 with one-line purposes paraphrased from each file's own header, entries as backticked dir-relative paths (`shell/install.sh`), plus `tests/unit/docs/test_scripts_readme_catalog.py` deriving ground truth from `git ls-files` with a fail-loud empty-population assert; prose bullets only because `validate_readme_commands.py` executes README code-block commands |

## Tags

`#executable-convention` `#invariant-guard` `#exit-code-collision` `#fail-safe` `#read-only-verification` `#log-line-anchoring` `#observability` `#cli-verify-mode` `#ci-gate` `#bidirectional-invariant` `#mirror-parity` `#allowlist-with-rationale` `#test-structure` `#commit-trailer` `#dco-signoff` `#precommit-hook` `#sibling-checker-mirror` `#coverage-altitude` `#negative-branch-test` `#argparse-default-branch` `#nogo-tightening` `#inferred-data-honesty` `#doc-table-code-parity` `#membership-only-guard` `#guard-scope-population` `#partial-compliance` `#scope-lock-test` `#ghost-directory-guard` `#prefiltered-population-fixture` `#install-smoke-test` `#self-test-in-guard` `#imported-location` `#find-package` `#canonical-check-name` `#ecosystem-ci-board` `#manifest-repo-install` `#check-run-on-main` `#real-check-vs-na` `#canonical-check-run` `#release-packaging` `#least-privilege-workflow` `#sigpipe-safe` `#action-pin-verification` `#dry-run-release-check` `#trusted-publishing-oidc` `#pipeline-validator` `#docs-catalog-completeness` `#git-ls-files-ground-truth` `#readme-command-validator-trap` `#backticked-path-match` `#projectscylla` `#myrmidons` `#hephaestus`
