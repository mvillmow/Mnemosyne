...
24. Validate the runtime behavior of newly permitted boundary values, not only construction.
    PR #2675 / issue #2396 initially accepted `max_delay=0`, but a retrying call still slept
    `0.1s` because the minimum-sleep floor was applied after the cap. The exact-head review
    correctly kept NOGO until a same-head reply moved capping after the floor and added a retry
