...
- Generated PR prose says tests were not run, but live required checks may still establish merge readiness; the prose must not be treated as a check result.
...
23. Separate generated test prose from live merge readiness. A PR body saying “Testing Not run
    by the automation pipeline” is an automation receipt, not evidence that repository checks
    failed or were skipped. Audit live checks separately for merge readiness, while keeping them
    outside GO authority. If the review path emits no GitHub review or issue-comment object,
    record that observability fact and use the final head, durable GO label, required-check
    completion, and merge event as the audit trail. PR #2663 / issue #2381 followed this path.
...
| Reuse a `COMMENTED` review from a superseded head | PR #2666 / issue #2384 had an earlier review on `e811636c` before a force-push moved the PR to `050c52eb`. | The earlier review no longer described the final source, and `COMMENTED` review state is evidence rather than loop authorization. | Compare every review `commit_id` with the final head; retain superseded reviews as history only, require a final-head review, and use the exclusive GO/NOGO label transition as the durable decision. |
...
| PR #2666 / issue #2384 audit | The first `COMMENTED` review targeted superseded head `e811636c`; after force-push it remained historical only. The final review matched `050c52eb`; GO replaced NOGO, the slow `unit-tests` and `required-checks-gate` completed at `09:44:30Z` / `09:44:36Z`, and `merge_wait` conditionally merged `11a0c65c` at `09:45:06Z` with native auto-merge unset. |
...
| ProjectHephaestus | Issue #2384 / PR #2666 | Verified-ci superseded-review path. The first `COMMENTED` review bound to `e811636c` was not reused after the force-push; final-head review bound to `050c52eb`, GO replaced NOGO, slow required checks completed, and merge-wait conditionally merged the exact reviewed head. |
