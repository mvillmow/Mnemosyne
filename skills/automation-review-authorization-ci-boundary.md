...
- A force-push supersedes an earlier reviewed head; the superseded `COMMENTED` review is history only,
  and only a fresh final-head review with the exclusive GO/NOGO transition authorizes the merge.
...
| Direct implementation superseded-review audit | For issue #2384 / PR #2666, the first `COMMENTED` review targeted superseded head `e811636c`; after the force-push it remained historical only. The final review matched `050c52eb`; GO replaced NOGO at `09:35:54Z` / `09:35:56Z`, `required-checks-gate` completed at `09:44:36Z`, and `merge_wait` conditionally merged `11a0c65c` at `09:45:06Z` with native auto-merge unset. This is an audit of the current queue path, not validation of the proposed operator-authorization design above. |
...
| ProjectHephaestus | Issue #2384 / PR #2666 | verified-ci superseded-review audit. An earlier `COMMENTED` review on head `e811636c` became historical after a force-push; only the fresh review of final head `050c52eb` supported GO. GO replaced NOGO, required checks completed, and merge-wait conditionally merged `11a0c65c` with native auto-merge unset. |
