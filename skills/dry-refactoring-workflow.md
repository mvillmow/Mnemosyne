...
# --- METRICS_SERVICE-STYLE behavior-preserving duplicate cleanup ---
rg -n "class _FakeRequest|class _FakeApp|def route\(" tests/metrics_service/server tests/project/release -g "*.py"
rg -n "class (Flatten|Reshape|Permute|Transpose|View)Kernel|estimate_layout_reindex" metrics_service/metrics/ops -g "*.py"
rg -n "def _required_mapping|def _required_list|def _required_string|def _as_mapping" metrics_service/*validation.py
...
./.venv/bin/python -m ruff check metrics_service scripts tests --no-cache
./.venv/bin/pytest -q
./.venv/bin/python -m compileall metrics_service scripts tests
git diff --check
```
...
### Metrics Service v1.6.0 Local Verification

| Check | Result |
| ----- | ------ |
| Ruff | `./.venv/bin/python -m ruff check metrics_service scripts tests --no-cache` passed |
| Full pytest | `1249 passed, 6 skipped` locally and again in the pre-push hook |
| Compileall | `./.venv/bin/python -m compileall metrics_service scripts tests` passed |
| Diff hygiene | `git diff --check` passed |
| PR | example-org/metrics-service PR #908 opened; CI pending at capture time |
...
| example-org/metrics-service | PR #908 | Consolidated route-test fakes, layout-only metric kernels, validation field checks, and removed stale `hf_checkpoint_architecture_html.py` | verified-local — full local/pre-push test suite passed; PR CI pending at capture time |
