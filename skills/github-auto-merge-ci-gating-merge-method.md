...
tags:
  ...
  - stale-auto-merge-arm
  - pre-armed-prs
  - gated-sweep
  - merge-method-mismatch-arm
...
| 2026-07-16 | Added the stale pre-armed auto-merge audit for gated review sweeps, from a 2026-07-16/17 HomericIntelligence 50-PR gated sweep: (1) arms from prior sessions survive indefinitely (and survive force-pushes) and fire the instant checks turn green — 19 pre-armed PRs found mid-session, several merged before their review verdicts landed; (2) `mergeMethod: MERGE` arms on `required_linear_history` repos can NEVER complete (ProjectProteus #206, ProjectKeystone #603) — disarm and re-arm `--squash`; (3) when mass-disarm is not permitted, race the reviews with verdict-backed targeted disarms; (4) persistent "clean status" arm errors on CLEAN PRs fall back to direct `gh pr merge --squash`; (5) `squash_merge_commit_message=COMMIT_MESSAGES` bakes unreviewed commit-message claims into main unless merged directly with `--subject`/`--body` | verified-local — observed live in production GitHub during the sweep; this amendment's own CI not yet green at capture |
...
**Verification: verified-ci** (most failure modes observed and fixed live; the advisory-split and per-issue arming refinements are verified-local where noted; the v1.7.0 stale pre-armed-arm gated-sweep additions are verified-local).
...
- You are starting ANY gated review sweep — auto-merge arms from prior sessions (stale arms) survive indefinitely, survive force-pushes, and fire the instant checks turn green; scan every repo for pre-armed PRs FIRST and treat each as a gate-bypass risk.
- `autoMergeRequest.mergeMethod` is `MERGE` on a repo with `required_linear_history`/`non_fast_forward` — that arm can NEVER complete (GitHub rejects at merge time; the arm sits forever); disarm and re-arm `--squash`.
- Mass-disarming pre-existing armed PRs is not permitted (they are prior user state) — audit armed PRs before their CI settles and disarm individually only on NO-GO verdicts.
- `gh pr merge --auto` on a CLEAN PR keeps erroring "Pull request is in clean status" across retries — the PR has nothing left to wait for; fall back to a direct `gh pr merge --squash`.
- The repo sets `squash_merge_commit_message=COMMIT_MESSAGES` and an armed PR's commit messages carry unreviewed or false claims — merge directly with `--subject`/`--body` to control the recorded message; nothing else corrects it without a history rewrite.
...
- **"Pull request is in clean status"** — API internal state has not caught up on a freshly-CLEAN PR. **Retry immediately**; the second call seconds later succeeds with no other change. If the error PERSISTS across retries, the PR is simply ready to merge NOW — auto-merge has nothing left to wait for and GitHub refuses the arm; fall back to a direct `gh pr merge --squash` (which also lets you pass `--subject`/`--body`, the only way to control the recorded message on a `squash_merge_commit_message=COMMIT_MESSAGES` repo).
