...
- **(v1.6.0, verified-ci)** Inference Service PRs report `mergeStateStatus=DIRTY` and
...
- **(v1.7.0, verified-ci)** Inference Service `validate` fails after CLI simplification on
...
WT=/tmp/Inference Service-claude-config
...
Conflict-resolution examples from the verified Inference Service follow-up:
...
### Phase 1c (v1.7.0, verified-ci): Inference Service Endpoint-Only InferenceX and Workflow Drift

Use this when an Inference Service PR has a current-head `validate` failure after CLI simplification,
...
- Truncated logs pointed at a stale `--manifest` parser theory, but the full traceback showed
  `inference_service/__init__.py` calling `_detect_cluster()` from `main()` because `args.cluster`
  was omitted.
- The failing tests were endpoint-only `inferencex-benchmark --endpoint ... --dry-run` cases:
  `tests/test_inferencex_integration.py::test_inferencex_benchmark_cli_endpoint_only_dry_run_defaults_to_tmp_performance_harness`
  and
...
2. Add regression tests that monkeypatch `inference_service._detect_cluster` to `pytest.fail(...)`
...
| Trust truncated logs for Inference Service PR #255 validate | Read a shortened failure excerpt and chased a stale `--manifest` parser hypothesis | The full pytest traceback showed `inference_service/__init__.py` calling `_detect_cluster()` because `args.cluster` was omitted | Pull the full job log with `gh run view --job <job-id> --log`, then filter around `FAILED`, `Traceback`, and the command under test |
...
### Inference Service Mergeability Rebase-Before-Validate Parameters (v1.6.0)

```bash
# Live-state check before editing. Do not rely on gh pr checks alone.
gh pr view <number> --repo example-org/inference-service \
  --json headRefOid,mergeStateStatus,mergeable,statusCheckRollup,url
gh pr checks <number> --repo example-org/inference-service

# Rebase the conflicting PR branch before waiting for validate.
git fetch origin master <branch>
git worktree add /tmp/Inference Service-<branch-or-pr> <branch>
cd /tmp/Inference Service-<branch-or-pr>
git rebase origin/master
...
Inference Service branch mapping from the verified session:
...
| #157 | `move-inference_service-module` | CI passed after TOML coverage config fix; PR merged |

### Inference Service Endpoint-Only InferenceX Validate Parameters (v1.7.0)
...
INFERENCE_SERVICE=.venv/bin/inference_service PYTHON=.venv/bin/python scripts/validate.sh
...
| GitHub checks after push | `gh pr checks 255 --repo example-org/inference-service` showed all pass, including `validate` |
| Mergeability after push | `gh pr view 255 --repo example-org/inference-service --json mergeStateStatus,mergeable,url` returned `mergeStateStatus: CLEAN`, `mergeable: MERGEABLE`, URL `https://github.com/example-org/inference-service/pull/255` |
