...
#### S. Stale base predates a newly-added head-bound merge-authorization gate — the gate ImportErrors, rebase to inherit it

**Trigger**: A long-open automation PR fails a required merge-authorization / strict-review gate with a Python `ModuleNotFoundError` — e.g. `No module named hephaestus.automation.strict_review_proof`. The signal is an **ImportError from the GATE's own entrypoint**, not a `<<<<<<<` conflict and not a failed test assertion. The branch is several commits behind `main`. Its BASE SHA predates the commit that ADDED the gate module to `main`, and the gate is **head-bound** — it runs against the branch/base tree, where that module does not exist yet — so the gate cannot import itself and hard-fails before doing any review work. The same stale base frequently ALSO shows `mergeStateStatus=CONFLICTING`/`DIRTY` from unrelated infrastructure that main advanced past meanwhile (a `.pre-commit-config.yaml` header, a tooling-migration file, a lockfile).

**Concrete case (ProjectHephaestus PR #2217, issue #2152, branch `2152-auto-impl`)**: The strict-review-proof merge-authorization gate failed with `No module named hephaestus.automation.strict_review_proof`. The PR's base SHA (`7edaf6f7`) was **12 commits behind main** and predated BOTH #2242 (which introduced the head-bound `strict_review_proof` gate module) and #2236 (the uv tooling migration). The stale base also left the PR CONFLICTING/DIRTY with a `.pre-commit-config.yaml` header conflict against the new uv-era version-pinning model.

**Diagnosis (cheap, read-only) — distinguish this from "a real code bug on the branch":**

```bash
# 1. How far behind is the branch? (base = merge-base with main)
git fetch origin --quiet
git rev-list --count "$(git merge-base HEAD origin/main)"..origin/main   # commits the branch is missing

# 2. Prove the "missing module" is a MAIN-only file the base never had, not a branch defect.
git ls-tree -r --name-only origin/main | grep strict_review_proof         # exists on current main
git ls-tree -r --name-only "$(git merge-base HEAD origin/main)" | grep strict_review_proof   # ABSENT at the base
git log --oneline -1 origin/main -- <path/to/gate_module.py>              # the commit that ADDED it (post-dates the base)
```

If the module exists on `origin/main` but is absent at the merge-base, the gate is failing on **the base tree, not on anything the PR did** — the branch is simply too old to carry the gate.

**Fix**: Rebase the branch onto CURRENT `origin/main` so the trusted base revision contains the gate module; resolve any incidental config-drift conflict (here `.pre-commit-config.yaml`) in favor of main's newer model while preserving the PR's substantive change (here the mandatory gitleaks staged-secret hook), then re-sign for DCO+GPG (see § B2) and force-with-lease.

```bash
git rebase origin/main            # brings strict_review_proof into the base; resolve the .pre-commit-config.yaml header hunk
# resolve → keep main's version-pinning header, keep the PR's gitleaks hook
git add .pre-commit-config.yaml && git rebase --continue
git rebase --exec 'git commit --amend --no-edit -s -S' origin/main   # DCO + GPG on every replayed commit (§ B2)
pre-commit run --all-files        # verify the whole-tree gate is green post-rebase
git push --force-with-lease
```

**Lesson**: When a *merge-authorization / strict-review* gate (not a lint, not a test) fails with `No module named …`, treat "the branch is behind main" as the FIRST hypothesis, not "someone deleted the module." Head-bound gates execute against the PR's base tree, so a base SHA older than the gate's introducing commit makes the gate structurally unable to import itself — the failure is about the branch's AGE, not its diff. The remedy is a rebase onto current main (which is also what clears the co-occurring CONFLICTING/DIRTY state from unrelated infra drift), NEVER vendoring or re-creating the gate module onto the stale branch. This is the mirror image of § M (a rebase that CLAIMS success but left the base stale): here the fix IS the rebase, and you must VERIFY afterward that `git merge-base HEAD origin/main == git rev-parse origin/main` so the gate module is genuinely in the base.
...
| HomericIntelligence/ProjectHephaestus | 2026-07-17, PR #2217 (issue #2152, branch `2152-auto-impl`, "[mino] Make generic local secret scanning mandatory"): the `strict-review-proof` merge-authorization gate failed with `No module named hephaestus.automation.strict_review_proof` — an ImportError from the GATE, not a conflict or test. Base SHA `7edaf6f7` was 12 commits behind main and predated #2242 (head-bound strict-review gate module) and #2236 (uv tooling migration); the stale base ALSO left the PR CONFLICTING/DIRTY on a `.pre-commit-config.yaml` header. FIX (§ S): rebased `2152-auto-impl` onto current main (`be12f92f`) so the trusted base contained `strict_review_proof`, resolved the config conflict in favor of the uv-era version-pinning model while keeping the mandatory gitleaks staged-secret hook, re-signed for DCO+GPG. Verified: full pytest `6406 passed` and `pre-commit run --all-files` green on the rebased tree; PR merged. | verified-ci |
