...
| **Verification** | unverified — this is a PLANNING learning. Nothing was executed. The precedent-script facts and the existing `pixi.toml` ledger entry were verified by READING the tree this session; the proposed check script, its hook wiring, and its unit test were NOT written or run. The contiguous-comment-block walk and the single-line task regex carry concrete false-positive / false-negative risks (see Failed Attempts). |
...
#    that a TOML parser DISCARDS. Read lines, regex the task value, walk the contiguous comment block.
PIP_AUDIT_TASK_RE = re.compile(r'^\s*pip-audit\s*=\s*"(?P<cmd>.*)"\s*$')   # NOTE: single-line only (risk below)
IGNORE_VULN_RE   = re.compile(r"--ignore-vuln(?:=|\s+)(?P<vid>[A-Z]+-\d{4}-\d+)")
REREVIEW_RE      = re.compile(r"#.*Re-review:")

def _ledger_block_for(lines, vuln_id):
    # walk UP from the line containing vuln_id over the contiguous leading-`#` block
    ...  # assumes vuln_id appears IN the comment paragraph AND Re-review: is in the SAME unbroken `#` block
...
# 6. Unit-test the pure helper WITHOUT packaging the script.
...
2. **Parse the ledger as TEXT, not via a TOML parser.** The `Re-review:` trigger and the
   ...
   `pip-audit = "..."` task value to extract the `--ignore-vuln <ID>` suppressions, and
   walk the contiguous `#`-comment block above/around each suppression to confirm a
   `Re-review:` line is present.
...
6. **Unit-test the pure helper via `sys.path.insert`.** Since `scripts/` is not an
   installed package, the test does `sys.path.insert(0, str(repo_root / "scripts"))` then
   imports the pure findings helper directly and asserts on its return value for a
   well-formed ledger (zero findings) and a malformed one (one finding). This exercises
   the logic without running the whole `main()` / subprocess.
...
| (ASSUMPTION — unverified) Contiguous-comment-block walk for the trigger line | `_ledger_block_for` walks UP from the line containing the vuln ID over the leading-`#` block, expecting the `Re-review:` line in the SAME unbroken `#` block | If the ledger formats a suppression across a paragraph separated by a BARE blank line (no `#`), the walk stops early and FALSE-POSITIVES (reports a documented suppression as un-triggered) | The walk assumes the vuln ID appears IN the comment paragraph AND `Re-review:` is in the same unbroken `#` block with no `#`-less gap — verify the real ledger formatting and harden the walk before trusting it |
| (ASSUMPTION — unverified) Single-line `PIP_AUDIT_TASK_RE` | `^\s*pip-audit\s*=\s*"(?P<cmd>.*)"\s*$` matches only a single-line task string | A multi-line / triple-quoted TOML string for the `pip-audit` task yields an EMPTY match → zero suppressions detected → FALSE-NEGATIVE (check passes while the ledger is undocumented) | A single-line regex over a TOML value is fragile; handle multi-line / triple-quoted strings or assert the task is single-line, or the gate silently no-ops |
| (ASSUMPTION — unverified) "the existing PYSEC-2025-183 entry passes" | Asserted the real `pixi.toml` suppression for PYSEC-2025-183 satisfies the new check | This was asserted from a FILE READ, not from running the script — the walk + regex were never executed against the real ledger | Run the script against the real `pixi.toml` before claiming the existing entry passes; a read is not an execution |
...
- **Parse strategy:** TEXT, not TOML — the reason + `Re-review:` metadata is comment-only and a TOML parser discards it. Read lines, regex the `pip-audit = "..."` task value to pull `--ignore-vuln <ID>` IDs, walk the contiguous `#` block for the `Re-review:` line.
...
- **Open risks (UNVERIFIED — do not trust without running):**
  - `_ledger_block_for` assumes the vuln ID appears IN the comment paragraph AND the `Re-review:` line is in the SAME unbroken `#` block (no bare blank line between). A paragraph separated by a `#`-less blank line stops the walk early → FALSE-POSITIVE.
  - `PIP_AUDIT_TASK_RE` matches only a single-line `pip-audit = "..."`. A multi-line / triple-quoted TOML string yields an empty match → zero suppressions detected → FALSE-NEGATIVE.
  - "The existing PYSEC-2025-183 entry passes" was asserted from a file READ, not from running the script.
- **Verification level:** unverified (plan-only; verified-local at best from reads). Nothing executed end-to-end.
